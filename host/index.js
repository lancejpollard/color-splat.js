...
export default function splat(color) {
    ...
    const colorRGBArray = colorConvert.hex.rgb(color);
    const colorRGB = {
        r: colorRGBArray[0],
        g: colorRGBArray[1],
        b: colorRGBArray[2],
    ...
    return {
        ...
        splitComplementary: colorHarmonizer.harmonize(color, 'splitComplementary'),
        ...
        colorSpaces: {
            hex: color,
            rgb: colorRGB,
            rgbPercent: colorRGBArray.map((x) => round((x / 255) * 100)),
            hsl: colorObject.hsl,
            hsv: colorObject.hsv,
            cieLab: colorObject.lab,
            xyz: colorObject.xyz,
            cieLch: colorObject.lch,
            binary: colorRGBArray.map((x) => x.toString(2).padStart(8, '0')),
        },
    };
}
export function generateRainbowColors() {
    const rainbow = [];
    let i = 0;
    while (i < 360) {
...
export function classifyColor(red, green, blue) {
...
export function getSimilarityScore(rgb1, rgb2) {
    return (Math.pow(rgb1.r - rgb2.r, 2) +
        Math.pow(rgb1.g - rgb2.g, 2) +
        Math.pow(rgb1.b - rgb2.b, 2));
}
export function pickClosestColor(rgb, namedColorList) {
